...
try:
    ...
    raise ImportError(
        "Libero library not installed. Please refer to its documentation for installation."
    ) from err
...
class LiberoAdapterWrapper(AdapterWrapper):
    ...
    def update_environment(self, action: np.ndarray | list) -> dict[str, Any]:
        """
        Backward-compatible alias for older callers that use update_environment().
        """
