...
the HuggingFace "AutoClasses" (e.g., those defined in `vla_src.openvla.prismatic.extern.hf_*`) for "native" use in `transformers``
...
from vla_src.openvla.prismatic.extern.hf.configuration_prismatic import PrismaticConfig
from vla_src.openvla.prismatic.extern.hf.modeling_prismatic import PrismaticForConditionalGeneration
...
def convert_prismatic_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    # Get `config.json` and `checkpoint_pt` -- mirrors logic in `vla_src.openvla.prismatic.models.load.py`
    ...
    # Instantiate & Add Pad to Tokenizer =>> following `vla_src.openvla.prismatic.models.materialize.get_llm_backbone_and_tokenizer`
    ...
    # Create Vision Backbone & Transform =>> following `vla_src.openvla.prismatic.models.materialize.get_vision_backbone_and_transform`
